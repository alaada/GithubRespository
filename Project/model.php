...
/**
 * Laeb andmebaasist kirjed valitud lehekulje kohta ja tagastab need massiivina.
 *
 * @param int $page Lehekulje number, mille kirjeid kuvada
 *
...
function model_load($page)
{
    ...
    $query = 'SELECT ID, Nimetus, Kogus FROM alaada__numbrid ORDER BY Kategooria ASC LIMIT ?,?';
...
function model_add($nimetus, $kogus)
{
    ...
    $query = 'INSERT INTO alaada__numbrid (Nimetus, Kogus) VALUES (?, ?)';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
 ...
 *
 * @return bool Tagastab true kui uuendamine onnestus
 */
function model_update($id, $kogus)
{
    global $l;

    $query = 'UPDATE alaada__numbrid SET Kogus=? WHERE ID=? LIMIT 1';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
 ...
 * Parool salvestatakse BCRYPT rasina.
 *
...
function model_user_add($kasutajanimi, $parool)
{
    ...
    $query = 'INSERT INTO alaada__kasutajad (Kasutajanimi, Parool) VALUES (?, ?)';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
...
function model_user_get($kasutajanimi, $parool)
{
    ...
    $query = 'SELECT ID, Parool FROM alaada__kasutajad WHERE Kasutajanimi=? LIMIT 1';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
