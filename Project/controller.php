...
function controller_add($nimetus, $kogus)
{
    ...
    // kontrollime kas sisendvaartused on oodatud kujul või mitte
    if ($nimetus == '' || $kogus <= 0) {
    ...
    message_add('Andmete lisamine ebaonnestus');
...
function controller_delete($id)
{
    ...
    message_add('Broneering ebaonnestus');

    return false;
}

// Uuendab kirje vaartust
function controller_update($id, $kogus)
{
    ...
    message_add('Andmete uuendamine ebaõonnestus');
...
function controller_register($kasutajanimi, $parool)
{
    ...
    message_add('Konto registreerimine ebaonnestus, kasutajanimi voib olla juba voetud');
...
function controller_logout()
{
    ...
    message_add('Oled nuud välja logitud');
...
// Tagastab kõik hetkel ootel olevad sonumid
